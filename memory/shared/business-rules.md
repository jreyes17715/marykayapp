...
- **INACTIVE:** Determinado por flag backend `_kit_activa_confirmada` (plugin kit-restrictor). Si flag existe y es falsy → INACTIVE. Si null → confia en `consultant_state`. El frontend NO recalcula meses. Se evalua via resolveRestrictionState().
...
- **Flujo completo:**
  1. Estado INACTIVE (determinado por `_kit_activa_confirmada` = falsy) → checkout exige 20,000 en seccion 2
  2. `validarCarrito` usa `calcularTotalSeccion2(cartItems, user)` para verificar minimo
  3. Compra >= 20,000 (seccion 2) se aprueba
  4. `handleOrderSuccess` → `buildMetaUpdatesForTransition('active', { fromInactive: true })` → escribe `consultant_state='active'`, `_kit_last_active_purchase_ts=<now>`, `_kit_activa_confirmada='1'`
  5. `refreshUserData` reconstruye user → `_kit_activa_confirmada='1'` → `restrictionState = null` → minimo vuelve a 1,000
- **Garantias:** La validacion de 20,000 NO se repite despues de reactivacion. `resolveRestrictionState` lee `_kit_activa_confirmada='1'` y no marca INACTIVE.
- **Razón:** Permite reactivacion organica sin intervencion de soporte, a diferencia de DISABLED.
- **Desde:** 2026-04-10 (seccion-2-only fix 2026-04-13, backend flag 2026-04-13)
